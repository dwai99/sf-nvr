...
class StorageManager:
    ...
    def check_and_cleanup(self, protected_paths: Optional[Set] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Storage usage: {usage_percent:.1f}% ({disk.used / (1024**3):.1f} GB / {disk.total / (1024**3):.1f} GB)"
            )
            ...
            if usage_percent < self.cleanup_threshold and not reserved_violated:
                logger.info(
                    f"Disk usage ({usage_percent:.1f}%) below threshold ({self.cleanup_threshold}%), free space {free_gb:.1f} GB, no cleanup needed"
                )
                return stats

            if reserved_violated:
                logger.warning(
                    f"Free space ({free_gb:.1f} GB) below reserved minimum ({self.reserved_space_gb} GB), starting cleanup"
                )

            stats["cleanup_triggered"] = True
            logger.warning(
                f"Disk usage ({usage_percent:.1f}%) exceeds threshold ({self.cleanup_threshold}%), starting cleanup"
            )
        ...
        try:
            ...
            for video_file in self.storage_path.rglob("*.mp4"):
                if video_file.is_file():
                    ...
                    files.append(
                        {"path": video_file, "size": stat.st_size, "mtime": datetime.fromtimestamp(stat.st_mtime)}
                    )
            ...
            logger.info(
                f"Need to free {space_to_free / (1024**3):.2f} GB to reach {self.target_percent}% usage (reserved: {self.reserved_space_gb} GB)"
            )
            ...
            for file_info in files:
                ...
                if bytes_freed >= space_to_free:
                    logger.info("Target disk usage reached, stopping cleanup")
                    break
                        ...
                        try:
                            ...
                            for seg in segments:
                                if filename in seg.get("file_path", ""):
                                    segment_start = (
                                        datetime.fromisoformat(seg["start_time"]) if seg.get("start_time") else None
                                    )
                                    segment_end = (
                                        datetime.fromisoformat(seg["end_time"]) if seg.get("end_time") else None
                                    )
                    ...
                    if self.playback_db:
                        try:
                            ...
                            if segment_start and segment_end:
                                self.playback_db.delete_motion_events_in_range(camera_id, segment_start, segment_end)
    ...
    def get_retention_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            "total_files": 0,
            "total_size_gb": 0.0,
            "files_by_age": {"<1day": 0, "1-3days": 0, "3-7days": 0, ">7days": 0},
