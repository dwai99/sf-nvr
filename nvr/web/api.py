...
async def startup_event():
    ...
    sd_card_manager = SDCardRecordingsManager(
        playback_db=playback_db, cache_duration=sd_card_config.get("cache_duration_seconds", 300), query_timeout=30.0
    )
    ...
    logger.info(
        f"Storage manager initialized: {storage_manager.retention_days} day retention, cleanup at {storage_manager.cleanup_threshold}%"
    )
            ...
            while True:
                try:
                    ...
                    if stats["cleanup_triggered"]:
                        logger.info(
                            f"Storage cleanup completed: deleted {stats['files_deleted']} files, freed {stats['space_freed_gb']:.2f} GB"
                        )
                except Exception as e:
            ...
            while True:
                try:
                    ...
                    for camera_health in response:
                        await alert_system.check_camera_health(camera_health["camera_name"], camera_health)

                    # Check storage capacity
                    import psutil

                    disk = psutil.disk_usage(str(config.storage_path))
                    await alert_system.check_storage(disk.percent, disk.free / (1024**3))
                    ...
                    await alert_system.check_storage_writable(config.is_storage_writable())
    ...
    if use_ai:
        ai_monitor = AIDetectionMonitor(confidence_threshold=config.get("ai_detection.confidence_threshold", 0.5))
    ...
    if config.get("onvif.auto_discover", True):
        ...
        try:
            ...
            discovery = ONVIFDiscovery(username=config.default_camera_username, password=config.default_camera_password)
    ...
    if config.get("recording.high_res_on_motion", False) and playback_db:
        ...
        event_recorder = HighResEventRecorder(
            ...
            max_seconds_per_hour=config.get("recording.high_res_max_seconds_per_hour", 600),
        )
...
async def disk_monitor_task():
    ...
    logger.info(
        f"Disk monitor started: min_free={MIN_FREE_GB}GB, warning={WARNING_THRESHOLD}%, critical={CRITICAL_THRESHOLD}%"
    )
    ...
    while True:
        try:
            ...
            needs_cleanup = (
                usage["percent"] >= CRITICAL_THRESHOLD
                or usage["free_gb"] < MIN_FREE_GB
                or usage["percent"] >= WARNING_THRESHOLD
            )
...
async def get_cameras(response: Response) -> List[Dict[str, Any]]:
    ...
    for camera in cameras:
        ...
        is_recording = (
            (recorder is not None and recorder.is_recording and not recorder.streaming_only) if recorder else False
        )
    ...
    return {
        ...
        "health_metrics": {
            ...
            "last_connection_attempt": (
                recorder.last_connection_attempt.isoformat() if recorder.last_connection_attempt else None
            ),
            "last_successful_connection": (
                recorder.last_successful_connection.isoformat() if recorder.last_successful_connection else None
            ),
        ...
        "recording_info": {
            "current_segment_start": (
                recorder.current_segment_start.isoformat() if recorder.current_segment_start else None
            ),
...
async def stop_camera(camera_id: str, permanent: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        camera_name = recorder.camera_name
        logger.info(
            f"Recorder state before stop: is_recording={recorder.is_recording}, streaming_only={recorder.streaming_only}"
        )
        ...
        logger.info(
            f"Recorder state after start: is_recording={recorder.is_recording}, streaming_only={recorder.streaming_only}"
        )
...
async def get_recording(camera_id: str, filename: str):
    ...
    try:
        ...
        return FileResponse(resolved_file, media_type="video/mp4", filename=resolved_file.name)
...
async def get_storage_stats():
    ...
    for camera in config.cameras:
        ...
        if not camera_dir.exists():
            camera_stats.append({"camera_name": camera["name"], "camera_id": camera_id, "size_gb": 0, "file_count": 0})
            continue
    ...
    return {"cameras": camera_stats, "total_size_gb": round(total_size, 2)}
        ...
        while True:
            ...
            if jpeg_data is not None:
                ...
                is_valid_jpeg = (
                    ...
                    and jpeg_data[0:2] == b"\xff\xd8"  # Start of Image
                    and jpeg_data[-2:] == b"\xff\xd9"  # End of Image
                )
                ...
                if is_valid_jpeg:
                    ...
                    if frames_sent > 0 and frames_sent % 100 == 0:
                        logger.warning(
                            f"WebSocket skipping corrupted frame for {camera_name} (size: {frame_size}, last_good: {last_frame_size})"
                        )
            else:
                no_frame_count += 1
...
async def run_manual_cleanup():
    ...
    try:
        ...
        return {
            ...
            "message": (
                f"Deleted {stats['files_deleted']} files, freed {stats['space_freed_gb']:.2f} GB"
                if stats["cleanup_triggered"]
            ...
            formatted.append(
                {
                    ...
                    "file_size_mb": (
                        round(entry["file_size_bytes"] / (1024 * 1024), 1) if entry["file_size_bytes"] else 0
                    ),
...
async def get_motion_heatmap(camera_id: str, date: str = None):
    ...
    try:
        ...
        return FileResponse(
            heatmap_path, media_type="image/png", filename=f"{camera_name}_heatmap_{date or 'today'}.png"
        )
...
async def get_system_stats():
    ...
    return {
        "cpu": {"percent": round(cpu_percent, 1), "cores": cpu_count},
        "memory": {
